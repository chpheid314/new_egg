...
type DocumentData = {
  ...
  categoryId: string;
  title: string;
  thumbnail: string; // 문서 썸네일 경로(일단 임시로 검색 버튼 사용했음)
  link: string; // 문서 상세 페이지 링크
};
...
const Document: React.FC<DocumentProps> = ({ data }) => {
  return (
    <a href={data.link} className="h-[60px] bg-white rounded-md relative">
      <img
        src={data.thumbnail}
        className="absolute top-1/2 left-[10px] w-[40px] h-[40px] -translate-y-1/2"
      />
      <div className="absolute top-1/2 left-[60px] -translate-y-1/2 font-bold">
        {data.title}
      </div>
    </a>
  );
...
function Class() {
  ...
  return (
    <div className="bg-[#F8D9FF] p-2 w-full h-full overflow-auto relative">
      <div
        className="absolute top-[50px] left-1/2 w-[960px] grid gap-x-8 -translate-x-1/2"
        style={{
          gridTemplateColumns: `repeat(${categories.length}, minmax(0, 1fr))`,
        }}
      >
        {categories.map((cat) => (
          <div key={cat.id} className="flex flex-col gap-y-5">
            <div className="bg-[#AF55C4] h-[48px] rounded-md font-bold text-[#FCF1FF] text-xl flex items-center justify-center">
              {cat.label}
            </div>
            {documents
